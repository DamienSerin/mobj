...
namespace Netlist {
  ...
  void  Cell::toXml ( std::ostream& s) {
    int x;
    s<<indent<<"<?xml version=\"1.0\"?>"<<endl;
    s<<indent<<"<cell name=\""<<name_;s<<"\">"<<endl;
    //Terms
    indent++;
    s<<indent<<"<terms>"<<endl;
    indent++;
    for(x=0;x<terms_.size();x++){
      if(terms_[x]){terms_[x]->toXml(s);}
    }
    indent--;
    s<<indent<<"</terms>"<<endl;
    //Instances
    s<<indent<<"<instances>"<<endl;
    indent++;
    for(x=0;x<instances_.size();x++){
      if(instances_[x]){instances_[x]->toXml(s);}
    }
    indent--;
    s<<indent<<"</instances>"<<endl;
    //Nets
    s<<indent<<"<nets>"<<endl;
    indent++;
    for(x=0;x<nets_.size();x++){
      if(nets_[x]){nets_[x]->toXml(s);}
    }
    indent--;
    s<<indent<<"</nets>"<<endl;
    indent--;
